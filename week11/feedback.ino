...
void loop() {
  ...
  if(Serial.available()){
    String motorspeed = Serial.readString();
    setSpeed(motorspeed.toInt());
    Serial.println(count);
  }
  delay(200);
}
...
void checkEdge(){
	bool curA = digitalRead(2);
  	bool curB = digitalRead(3);
    if ((pre_read_A == 1) && (curA == 0)){
      count--;
      state = 0;
    }
  	if ((pre_read_B == 1) && (curB == 0)){
      count++;
      state = 1;
  	}
  	pre_read_A = curA;
